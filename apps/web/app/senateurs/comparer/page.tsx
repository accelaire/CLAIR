...
function ComparerPageContent() {
  ...
  const { data, isLoading, error } = useQuery<CompareResponse>({
    queryKey: ['senateurs-compare', slugs],
    queryFn: () =>
      api.get('/senateurs/compare', { params: { slugs: slugsParam } }).then((res) => res.data),
  ...
  const handleRemove = (slug: string) => {
    ...
    if (newSlugs.length >= 2) {
      router.push(`/senateurs/comparer?slugs=${newSlugs.join(',')}`);
    } else {
      router.push('/senateurs');
    }
  };
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="rounded-lg border border-orange-500/50 bg-orange-500/10 p-6 text-center">
          ...
          <p className="mt-2 text-muted-foreground">
            Veuillez sélectionner au moins 2 sénateurs pour les comparer.
          </p>
          <Link
            href="/senateurs"
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <h1 className="mt-2 text-3xl font-bold">Comparaison de sénateurs</h1>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {parlementaires.map((p) => (
              ...
              >
                ...
                <Link href={`/senateurs/${p.slug}`} className="block">
                  <div className="flex flex-col items-center text-center">
                    ...
                    {/* Circonscription ou série */}
                    {p.circonscription ? (
                      <p className="mt-1 text-xs text-muted-foreground">
                        {p.circonscription.nom}
                      </p>
                    ) : p.serie && (
                      <p className="mt-1 text-xs text-muted-foreground">
                        Série {p.serie}
            ...
            {parlementaires.length < 4 && (
              <Link
                href={`/senateurs?compare=${slugs[0]}`}
              ...
              >
                ...
                <span className="mt-2 text-sm">Ajouter un sénateur</span>
              </Link>
            )}
          </div>
